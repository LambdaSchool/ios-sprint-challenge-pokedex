...
class SearchDetailViewController: UIViewController {
    ...
    var pokeapiController: PokeAPIController? {
        didSet {
            updateViews()
        }
    }
...
override func viewDidLoad() {
    ...
    @IBAction func saveButtonTapped(_ sender: UIButton) {
        ...
        pokeapiController?.addPokemon(pokemon: pokemon)
        navigationController?.popViewController(animated: true)
    }
    
    @IBAction func deleteButtonTapped(_ sender: UIButton) {
        guard let pokemon = pokemon else {return}
        pokeapiController?.delete(pokemon: pokemon)
        navigationController?.popViewController(animated: true)
    }
    ...
    func updateViews() {
        ...
        pokeapiController?.fetchImage(from: pokemonObject.sprites.imageUrl, completion: { (pokemonImage) in
...
extension SearchDetailViewController: UISearchBarDelegate {
     func searchBarSearchButtonClicked(_ sender: UISearchBar) {
        guard let searchTerm = searchBarOutlet.text else { return }
         ...
         pokeapiController?.fetchPokemon(pokemonName: searchTerm) { (pokemonObject) in
        ...
        pokeapiController?.fetchImage(from: pokemonImgUrl, completion: { (pokemonImage) in
            DispatchQueue.main.async {
                self.imageView.image = pokemonImage
            }
        })
